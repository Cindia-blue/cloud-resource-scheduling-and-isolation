...
func CheckEngine(engine int) bool {
    return (GetAgent().EnginesSwitch & engine) > 0
}
...
func (c *Agent) InitClient() error {
    ...
    if kubeconfig != "" {
        ...
        if err != nil {
            klog.Warning("📦 Failed to load config from kubeconfig file:", err)
            return err
        }
    } else {
        ...
        if err != nil {
            klog.Warning("📦 Failed to load in-cluster config:", err)
            return err
        }
    }
    ...
    if err != nil {
        klog.Warning("Clientset init err: " + err.Error())
        return err
    }
...
func init() {
    ...
    for k, v := range node.Labels {
        ...
        if k == CsiSeperatedLabel && v == "true" {
            csiEnable = false
        }
    }

    klog.Infof("🔍 ioisolation resources parsed: %v", resources)

    for _, r := range resources {
        switch r {
        case "disk":
            c.EnginesSwitch |= DiskSwitch
            if csiEnable {
                c.EnginesSwitch |= CSISwitch
            }
        case "net":
            klog.Infof("Detected network control requirement: enabling NetSwitch")
            c.EnginesSwitch |= NetSwitch
        case "rdt":
            c.EnginesSwitch |= RDTSwitch
        case "rdtQuantity":
            c.EnginesSwitch |= RDTQuantitySwitch
        case "all":
            klog.Infof("Detected 'all' resource: enabling all switches")
            c.EnginesSwitch |= DiskSwitch | NetSwitch | RDTSwitch | RDTQuantitySwitch
            if csiEnable {
                c.EnginesSwitch |= CSISwitch
            }
        }
    ...
    klog.Infof("🔧 Final EnginesSwitch mask: %04b", c.EnginesSwitch)
}