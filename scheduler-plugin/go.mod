module sigs.k8s.io/IOIsolation/scheduler-plugin
...
// k8s.io/kubernetes's own go.mod replaces every staging module with a
// local ./staging/src path, which only resolves when building inside
// the kubernetes module itself. As an external consumer we replicate
// its replace set, pointing every staging module at the matching
// v0.34.9 published module instead, so the whole graph resolves to one
// consistent Kubernetes minor version.
replace (
